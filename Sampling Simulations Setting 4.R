...
#It'll have to be a fine enough resolution I guess
...
for (sub in 1:20){
  ...
  for (n in 5:nrep){
    ...
    while(TRUE){
      ...
      data<-list(A=big_A,H=big_H,H_i=H_i,H_j=H_j,
                 ...
                 hook_num=c(700,300),
                 non_stat=matrix(nrow=length(sf_grid),ncol=2))
      
      data$idx[,1]<-data$idx[,1]-1
      
      data$non_stat[bot_left_ids,1]<-3
      data$non_stat[-(bot_left_ids),1]<- -0.58
      data$non_stat[,2]<-data$non_stat[,1]
      ...
      #First 2 years
      sim_obj<-MakeADFun(data,par_list,random=random,DLL="nngp_updated_model_nonstat",map=map,silent=F)
      ...
      for (t in 3:strat_n_t){
        ...
        new_obj<-MakeADFun(new_data,new_par_list,random=random,DLL="nngp_updated_subsim_nonstat",map=map,silent=F)
      ...
      plot_strats_frame<-cbind(sf_grid3[,2],strat_list[[n]]$strat_sim_data$A[-(1:main_n[1]),1]+strat_list[[n]]$strat_sim_data$H[-(1:main_n[1]),2],
      ...
      #Using my testing stuff to illustrate how to do this
      #Will take the mean catches in those 4 years
      ...
      sf_mean_frame<-data.frame(geometry=sf_grid2[strat_sim_data$idx[1:(main_n[1]),1]+1,1]$x,means=mean_vec) %>% st_as_sf()
      ...
      sf_spat_avg<-data.frame(geometry=sf_grid2[strat_sim_data$idx[1:(main_n[1]),1]+1,1]$x,means=spat_moving_avg) %>% st_as_sf()
      ...
      #This looks great! I think I can reproduce this a bunch
      #I can argue doing this because it creates heavily skewed data and cutting it evenly will give too few points
    ...
    for (t in 1:n_t_forward){
      ...
      new_data$non_stat[(bot_left_ids+((t-1)*51)),1]<-3
      new_data$non_stat[-(bot_left_ids+((t-1)*51)),1]<- -0.58
      new_data$non_stat[(bot_left_ids+((t)*51)),2]<-3
      new_data$non_stat[-(bot_left_ids+((t)*51)),2]<- -0.58
  ...
  save(strat_list,file=paste0("strat_list_set_4_",sub,".RData"))
  save(sub_data_list,file=paste0("sub_data_list_set_4_",sub,"RData"))
  save(fit_sim_stuff,file=paste0("fit_sim_stuff_set_4_",sub,".RData"))
  save(rand_list,file=paste0("rand_model_fit_set_4_",sub,".RData"))
  save(fixed_prop_list,file=paste0("fixed_prop_model_fit_set_4_",sub,".RData"))
  save(fixed_hal_alloc_list,file=paste0("fixed_hal_alloc_model_fit_set_4_",sub,".RData"))
  save(strat_prop_list,file=paste0("strat_prop_model_fit_set_4_",sub,".RData"))
  save(strat_hal_alloc_list,file=paste0("strat_hal_alloc_model_fit_set_4_",sub,".RData"))
}
