...
compile("nngp_updated_strata_eff.cpp")
dyn.load(dynlib("nngp_updated_strata_eff"))
...
for (sub in 1:20){
  ...
  for (n in 1:nrep){
    ...
    if (Opt_rob$message=="iteration limit reached without convergence (10)"){
  ...
  save(rob_list,file=paste0("rob_fit_set_4_fixed_",sub,".RData"))
}
...
for (sub in 1:20){
  ...
  save(rob_list,file=paste0("rob_fit_set_4_strat_",sub,".RData"))
}
