...
struct distribution {
  ...
  T operator() (T data,T mean,T size) {
    ...
    case 4 : return dgamma( // Gamma
        data,
        pow(mean / pars(0), 2), // shape = (mu / sd)^2
        pow(pars(0), 2) / mean, // scale = sd^2 / mu
        true
    );
...
public:
  ...
  Type operator() (Type x, Type lmean, Type sample_size) {
    return dist(x, ilink(lmean), sample_size);
  }
  Type simulate(Type lmean, Type sample_size) {
    return dist.simulate(ilink(lmean), sample_size);
  }
};