...
compile("nngp_updated_subsim2.cpp")
dyn.load(dynlib("nngp_updated_subsim2"))
...
for (sub in 1:20){
  ...
  for (n in 1:nrep){
    ...
    while(TRUE){
      ...
      for (t in 3:strat_n_t){
        ...
        new_obj<-MakeADFun(new_data,new_par_list,random=random,DLL="nngp_updated_subsim2",map=map,silent=F)
      ...
      plot_strats_frame<-cbind(sf_grid3,strat_list[[n]]$strat_sim_data$A[-(1:main_n[1]),1]+strat_list[[n]]$strat_sim_data$H[-(1:main_n[1]),2],
    ...
    #Setting 3: Keeping it consistent first 10 years, then drop range and increase variances afterwards, force population drops
    for (t in 1:n_t_forward){
      new_soaky<-c(prev_sim$s[(main_n[1]+1):sum(main_n)],rep(1,main_n[2]))
      new_big_A<-rbind(prev_sim$A[(main_n[1]+1):sum(main_n),],
                       matrix(rep(0.3,sum(main_n)/2*n_k),nrow=sum(main_n)/2,ncol=n_k))
      ...
      if (t >= 10){
        new_par_list$pre_pars_rands<-array(data=c(new_par_list$pre_pars_rands[1,1]-0.5,logitp((0.99+1)/2),log(0.5),-6,logitp((0.99+1)/2),log(0.5)),dim=c(3,2))
        new_par_list$pre_pars_omegas<-array(data=c(log(0.05),log(4),0,log(0.1),log(2),0),dim=c(3,2))
        new_par_list$rands[1:nrow(new_par_list$rands),]<-new_par_list$rands[1:nrow(new_par_list$rands),]
      }
  ...
  save(strat_list,file=paste0("strat_list_set_3_",sub,".RData"))
  save(sub_data_list,file=paste0("sub_data_list_set_3_",sub,"RData"))
  save(fit_sim_stuff,file=paste0("fit_sim_stuff_set_3_",sub,".RData"))
  save(rand_list,file=paste0("rand_model_fit_set_3_",sub,".RData"))
  save(fixed_prop_list,file=paste0("fixed_prop_model_fit_set_3_",sub,".RData"))
  save(fixed_hal_alloc_list,file=paste0("fixed_hal_alloc_model_fit_set_3_",sub,".RData"))
  save(strat_prop_list,file=paste0("strat_prop_model_fit_set_3_",sub,".RData"))
  save(strat_hal_alloc_list,file=paste0("strat_hal_alloc_model_fit_set_3_",sub,".RData"))
}
