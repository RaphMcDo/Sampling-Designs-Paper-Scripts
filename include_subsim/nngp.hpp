  ...
  for(int v = 0; v < pg.dim_v(); v++) {
    for(int t = 1; t < pg.dim_t(); t++) {
      for(int node = 0; node < pg.dim_g(); node++) {
        ...
        re_dag_node<Type> pg_node2 = pg(node, t, v);
        // "to" nodes for pg_node.re won't be used
        vector<Type> sim = pg_c(node, v).simulate(
          (t == 0 ? 1.0 / ts.initial_sd_scale(v) : 1.0 ) * pg_node2.re,
          (t == 0 ? 1.0 / ts.initial_sd_scale(v) : 1.0 ) * pg_node2.mean
        );
        sim.segment(0, pg_node2.node.to.size()) = (t == 0 ? ts.initial_sd_scale(v) : 1.0) * sim.segment(0, pg_node2.node.to.size());
